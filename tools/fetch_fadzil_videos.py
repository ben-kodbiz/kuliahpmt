#!/usr/bin/env python3
"""
Tool to fetch video IDs from Ustaz Fadzil's YouTube playlists.
"""
...
def fetch_fadzil_playlist_videos():
    """Fetch videos from Ustaz Fadzil's playlists."""
    
    # Base directory
    base_dir = "/data/work/pmt"
    ustaz_dir = os.path.join(base_dir, "ustaz", "fadzil")
    os.makedirs(ustaz_dir, exist_ok=True)
    
    # Ustaz Fadzil's playlists
    # Based on fadzil.html, there are 2 correct playlists:
    # 1. Tafsir Al-Quran 
    # 2. Ustaz Fadzil Kamaruddin (general collection)
    playlists = [
        {
        ...
        {
            'name': 'ustaz_fadzil_kamaruddin',
            'title': 'Ustaz Fadzil Kamaruddin',
            'url': 'https://www.youtube.com/playlist?list=PLv_xoi4FGsu-M0_mtXAE9cmKRXYRmEbrn',  # General collection
            'color': '#fff3e0',
            'icon': 'school'
        }
    ]
...
<head>
    ...
    <title>{playlist['title']} - Ustaz Fadzil Kamaruddin</title>
    <style>
        :root {{
...
def main():
    """Main function."""
    print("Fetching Ustaz Fadzil's YouTube playlist videos...")
    print("=" * 60)
    
    try:
        data = fetch_fadzil_playlist_videos()
        print("\n" + "=" * 60)
        print("SUCCESSFULLY FETCHED FADZIL'S VIDEOS!")
        print("=" * 60)
        print("Directories created:")
        for playlist_name in data.keys():
            print(f"- ustaz/fadzil/{playlist_name}/")
