#!/usr/bin/env python3
"""
Tool to fetch video IDs from Ustaz Halim Hasan's YouTube playlists.
"""
...
def fetch_halim_playlists():
    """Fetch videos from Ustaz Halim Hasan's playlists."""
    
    # Base directory
    base_dir = "/data/work/pmt"
    ustaz_dir = os.path.join(base_dir, "ustaz", "halim")
    os.makedirs(ustaz_dir, exist_ok=True)
    
    # Ustaz Halim Hasan's playlists from ustazhalim.txt
    playlists = [
        {
            'name': 'perubatan_tradisional_dan_adat_resam_orang_melayu',
            'title': 'Ustaz Halim Hassan: Perubatan Tradisional Dan Adat Resam Orang Melayu',
            'url': 'https://www.youtube.com/playlist?list=PLv_xoi4FGsu8vaDF8BN63grv7fOAiu4XI',
            'color': '#fff3e0',
            'icon': 'healing'
        },
        {
            'name': 'menilai_tarikat_dan_kesufian',
            'title': '2025 Ustaz Halim Hassan: Menilai Tarikat Dan Kesufian Dengan Syariat Allah',
            'url': 'https://www.youtube.com/playlist?list=PLv_xoi4FGsu_xmB5KRbY9F1W0jlgH0Axk',
            'color': '#e8f5e9',
            'icon': 'account_balance'
        },
        {
            'name': 'pengajian_mustolahul_hadis',
            'title': '2012 Ustaz Halim Hassan, Pengajian Mustolahul Hadis',
            'url': 'https://www.youtube.com/playlist?list=PLMYfcjZnGL4c73v9aQLd3-RoUZyE5Kx6j',
            'color': '#e3f2fd',
            'icon': 'menu_book'
        },
        {
            'name': 'ustaz_halim_hasan_2013',
            'title': 'Ustaz Halim Hassan 2013 - From Kay Hassan',
        ...
        },
        {
            'name': 'manasik_umrah',
            'title': 'Manasik Umrah',
            'url': 'https://www.youtube.com/playlist?list=PLoabBUZneY54e9VVgXGUqhSx6oavZ2djD',
            'color': '#e0f7fa',
            'icon': 'location_city'
        }
    ]
    ...
    for playlist in playlists:
        ...
        ydl_opts = {
            ...
            'quiet': False,
            'skip_download': True,
            # Remove the video limit to fetch all videos
        }
...
<head>
    ...
    <title>{playlist['title']} - Ustaz Halim Hasan</title>
    <style>
        :root {{
...
<body>
    <div class="header">
        ...
        <div class="stats">
            ...
            <div class="stat">
                <div class="stat-value">{format_duration(sum(v.get('duration') or 0 for v in videos))}</div>
...
def format_duration(seconds):
    ...
    if not seconds and seconds is not None:  # Handle both None and zero values properly
        return "N/A"
    if seconds is None:
        return "N/A"
    hours = seconds // 3600
...
def main():
    """Main function."""
    print("Fetching Ustaz Halim Hasan's YouTube playlist videos...")
    print("=" * 60)
    
    try:
        data = fetch_halim_playlists()
        print("\n" + "=" * 60)
        print("SUCCESSFULLY FETCHED HALIM HASAN'S VIDEOS!")
        print("=" * 60)
        print("Directories created:")
        print("- ustaz/halim/perubatan_tradisional_dan_adat_resam_orang_melayu/")
        print("- ustaz/halim/menilai_tarikat_dan_kesufian/")
        print("- ustaz/halim/pengajian_mustolahul_hadis/")
        print("- ustaz/halim/ustaz_halim_hasan_2013/")
